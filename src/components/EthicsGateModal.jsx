...
import './EthicsGateModal.css';

const guides = [
  {
    id: 1,
    badges: [
      { type: 'orange-outline', text: '💡 주도성' },
      { type: 'green-fill', text: '🎯 합목적성' }
    ],
    title: '가이드 1 활용 목적',
  ...
  },
  {
    ...
    desc: "생성형 AI는 틀린 정보를 마치 사실인 것처럼 제시하기도 하므로, 알려준 내용은 항상 '정말 맞을까?'하고 한 번 더 확인하는 습관을 가져요. 중요한 내용일수록 책을 찾아보거나 선생님께 여쭤보는 등 다른 방법으로 꼭 다시 확인하세요."
  },
  {
    id: 4,
    badges: [
  ...
  },
  {
    id: 6,
    badges: [
      { type: 'yellow-outline', text: '⚖️ 투명성' }
    ],
    title: '가이드 6 투명성·윤리',
...
function EthicsGateModal({ onClose, onAccept }) {
  return (
    <div className="modal-overlay" onClick={onClose} style={{ zIndex: 1000 }}>
      <div className="modal-card ethics-modal-card" onClick={(e) => e.stopPropagation()}>
        <div className="ethics-header">
          <div className="ethics-header-left">핵심 가치</div>
          <div className="ethics-header-right">핵심 가이드</div>
        </div>
        
        <div className="ethics-content-scroll">
          {guides.map(guide => (
            <div key={guide.id} className="ethics-row">
              <div className="ethics-badges">
                {guide.badges.map((badge, idx) => (
                  <span key={idx} className={`ethics-badge ${badge.type}`}>
                    {badge.text}
                  </span>
                ))}
              </div>
              <div className="ethics-text">
                <div className="ethics-title">{guide.title}</div>
                <div className="ethics-subtitle">{guide.subtitle}</div>
                <div className="ethics-desc">{guide.desc}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="ethics-footer">
          <button className="btn-primary ethics-accept-btn" onClick={onAccept}>
...
export default EthicsGateModal;
